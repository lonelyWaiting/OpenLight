...
bool MyApp::Init()
{
...

	{
		...
		result = md3dDevice->CreateVertexShader( VSFile.data() , VSFile.size() , nullptr , &pVertexShader );
		if( FAILED( result ) )
		{
			return false;
		}
		...
		result = md3dDevice->CreatePixelShader( PSFile.data() , PSFile.size() , nullptr , &pPixelShader );
		if( FAILED( result ) )
		{
			return false;
		}
		...
		result = md3dDevice->CreateInputLayout( InputLayoutDesc , size , VSFile.data() , VSFile.size() , &mInputLayout );
		if( FAILED( result ) )
		{
			return false;
		}
